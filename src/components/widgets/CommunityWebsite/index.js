...
const CommunityWebsiteWrapper = styled('div', {
...
export const CommunityWebsite = ({ details, handleDetails }) => {
    ...
    const handleWebsiteChange = website => {
        handleDetails({ ...details, website: website });
        form.resetFields();
        setHelp('');
    ...
    const onFinish = value => {
        ...
        if (isAuth()) {
            ...
            for (let i in value) {
                ...
                value[i] && websiteForm.append(i, value[i]);
            }

            websiteForm.append('username', JSON.parse(Cookies.get('auth_user')).username);

            axiosInstance.post(process.env.REACT_APP_BASE_URL + "community/update-website", websiteForm, {
                headers: {
            ...
            .then(response => {
                if (response.data.isSuccess) {
                    handleWebsiteChange(response.data.data.details.website);
                    showAlert();
                    setTimeout(() => {
                        message.success({
                            content: 'Community website updated.',
                            key,
                            duration: 2,
                            style: {
                                marginTop: '10vh',
            ...
            .catch(err => {
                if (err.response && err.response.data.errors) {
                    setHelp(<Text type="span" color="red">{err.response.data.errors.website && err.response.data.errors.website[0]}</Text>);
                }
            });
        } else {
        ...
        <CommunityWebsiteWrapper>
            <HeaderWrapper className="d-flex flex-row flex-sm-column flex-md-row justify-content-between align-items-center align-items-sm-stretch align-items-md-center">
                <LinkWrapper className="d-flex flex-wrap align-items-center" css={{ height: '100%', }}>
                    ...
                    text="Website" 
                    className="mt-1"
                    href={(details && details.website) && details.website} />
                </LinkWrapper>
