...
const CommunityTwitterWrapper = styled('div', {
...
export const CommunityTwitter = ({ details, handleDetails }) => {
    ...
    const handleTwitterChange = twitter => {
        handleDetails({ ...details, twitter_account: twitter });
        form.resetFields();
        setHelp('');
    ...
    const onFinish = value => {
        const twitterAccountForm = new FormData();
        ...
        if (isAuth()) {
            ...
            for (let i in value) {
                ...
                value[i] && twitterAccountForm.append(i, value[i]);
            }

            twitterAccountForm.append('username', JSON.parse(Cookies.get('auth_user')).username);

            axiosInstance.post(process.env.REACT_APP_BASE_URL + "community/update-twitter-account", twitterAccountForm, {
                headers: {
            ...
            .then(response => {
                if (response.data.isSuccess) {
                    handleTwitterChange(response.data.data.details.twitter_account);
                    showAlert();
                    setTimeout(() => {
                        message.success({
                            content: "Community's Twitter account updated.",
                            key,
                            duration: 2,
                            style: {
                                marginTop: '10vh',
            ...
            .catch(err => {
                ...
                if (err.response && err.response.data.errors) {
                    setHelp(<Text type="span" color="red">{err.response.data.errors.twitter_account && err.response.data.errors.twitter_account[0]}</Text>);
                }
            });
        } else {
            console.log('on camp twitter: no cookies');
        }
    }

    return (
        <CommunityTwitterWrapper>
            <HeaderWrapper className="d-flex flex-row flex-sm-column flex-md-row justify-content-between align-items-center align-items-sm-stretch align-items-md-center">
                <LinkWrapper className="d-flex flex-wrap align-items-center" css={{ height: '100%', }}>
                    ...
                    href={(details && details.twitter_account) && details.twitter_account} />
                </LinkWrapper>
